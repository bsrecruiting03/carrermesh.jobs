...
def profile_search(term="engineer"):
    conn = None
    try:
        conn = psycopg2.connect(settings.database_url)
        cur = conn.cursor()
        
        print(f"Profiling search for term: '{term}'")
        ...
        ORDER BY (ts_rank(j.search_vector, plainto_tsquery('english', %s)) + similarity(j.title, %s)) DESC
        LIMIT 20 OFFSET 0;
        """
