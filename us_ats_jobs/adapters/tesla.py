"""
Tesla Adapter - Uses SerpAPI Fallback
...
logger = logging.getLogger("TeslaAdapter")

class TeslaAdapter(PublisherAdapter):
    """Tesla Careers Adapter - Uses SerpAPI fallback."""
    
    def __init__(self):
        self.serpapi_adapter = SerpAPIJobsAdapter("tesla")
        logger.info("TeslaAdapter initialized (using SerpAPI fallback)")
        
    def fetch_jobs(self, cursor: Any = None) -> List[Dict]:
        raw_jobs = self.serpapi_adapter.fetch_jobs(cursor)
        return [normalize_serpapi_job(job, "tesla") for job in raw_jobs]
