"""
TikTok Adapter - Uses SerpAPI Fallback
...
class TiktokAdapter(PublisherAdapter):
    """TikTok Careers Adapter - Uses SerpAPI fallback."""
    
    def __init__(self):
        self.serpapi_adapter = SerpAPIJobsAdapter("tiktok")
        logger.info("TiktokAdapter initialized (using SerpAPI fallback)")
        
    def fetch_jobs(self, cursor: Any = None) -> List[Dict]:
        raw_jobs = self.serpapi_adapter.fetch_jobs(cursor)
        return [normalize_serpapi_job(job, "tiktok") for job in raw_jobs]
