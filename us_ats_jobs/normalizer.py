...
def normalize_amazon(job, company="Amazon"):
    """
    Normalizes Amazon job data.
    """
    # ID logic: Use id_icims if available, else id
    raw_id = job.get("id_icims") or job.get("id")
    
    # Description: Combine short description with qualifications if available
    desc_parts = [job.get("description_short", "")]
    ...
    # Link: Use url_next_step (Apply) or construct detail page
    # job_path: "/en/jobs/3110737/id-manager-ii-dcc-communities"
    job_path = job.get("job_path")
    if job_path:
        link = f"https://www.amazon.jobs{job_path}"
    else:
        link = job.get("url_next_step") or ""

    return {
        "job_id": f"amazon_{raw_id}",
        "title": job.get("title", ""),
        "company": company,
