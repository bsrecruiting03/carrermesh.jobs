"""
Apple Adapter - Uses SerpAPI Fallback
...
class AppleAdapter(PublisherAdapter):
    """Apple Careers Adapter - Uses SerpAPI fallback."""
    
    def __init__(self):
        self.serpapi_adapter = SerpAPIJobsAdapter("apple")
        logger.info("AppleAdapter initialized (using SerpAPI fallback)")
        
    def fetch_jobs(self, cursor: Any = None) -> List[Dict]:
        raw_jobs = self.serpapi_adapter.fetch_jobs(cursor)
        return [normalize_serpapi_job(job, "apple") for job in raw_jobs]
